...
class EsynergiFulfillmentService extends FulfillmentService {
  ...
  constructor({
    ...
    if (!options.coo_countries) {
      ...
      this.options_.coo_countries = options.coo_countries.map(c => c.toLowerCase());
  ...
  async getFulfillmentOptions() {// Fulfillment options from E-synergi: https://wms.e-synergi.dk/api-ext-v1/shipping-service
    // return data.items
  ...
  calculatePrice() {// Calculate prices
  }
  /**
  ...
  async createFulfillment(methodData, fulfillmentItems, fromOrder, fulfillment) {// create order in esynergi
